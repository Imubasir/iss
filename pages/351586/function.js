	...
	$.ajax({
		...
		success: function(json) {
			...
			for (var i = 0, a=1; i < data.length, a<data.length+1; i++, a++) {
				...
				var copies = data[i].copies;
				var prog = data[i].prog_name;
				var service = data[i].service_name;
				var verify_code = data[i].verify_code;

				var action = "<button class='btn btn-sm btn-primary' type='button' onclick='previewResults(\""+student_id+"\")'>Preview</button>";
                var printed = "<button class='btn btn-sm btn-success' type='button' onclick='printComplete(\""+trans_id+"\")'>Printed</button>";

                tbl_body+= "<tr><td>"+a+"</td><td>"+student_id+"</td><td>"+name+"</td><td>"+prog+"</td><td>"+service+"</td><td>"+copies+"</td><td>"+action+"</td><td>"+printed+"</td></tr>";
            }

            $("#trancript_body").html(tbl_body);
            $("#verify_code").html("*"+verify_code+"*");
            $("#trancript_table").dataTable();
        }
    })
}
...
function previewResults(id) {
	...
	$.ajax({
		...
		success: function (json) {
			...
			var programme = data['biodata'][0].prog_name;
			var grad_class = data['biodata'][0].grad_class;
...
function fetch_signatory () {
	$.ajax({
		url: 'fetch_signatory.php',
		...
		success: function(json) {
			...
			for (var i = 0; i < data.length; i++) {
				var sig_name = data[i].sig_name;
				var sig_desig = data[i].sig_designation;

				$("#trans_signatory").html(sig_name+"<br>("+sig_desig+")");
			}
		}
	})
}
...
function printComplete (id) {
    $("#trancript_table").dataTable().fnDestroy();
    $.ajax({
        type: 'POST',
        url: 'printComplete.php',
        data: 'id='+id,

        success: function (responseText) {
            if(responseText == '1') {
                new PNotify({
                    title: 'Transcript Printed',
                    text: '',
                    type: 'success',
                    styling: 'bootstrap3'
                })
